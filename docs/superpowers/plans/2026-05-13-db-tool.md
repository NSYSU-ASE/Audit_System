...
- [x] **Step 3：建立 ManifestFile record**
...
- [x] **Step 1：寫第一個失敗測試（合法 manifest 能讀進來）**
...
- [x] **Step 4：實作 ManifestLoader 的 happy path**
...
- [x] **Step 7：跑全部 ManifestLoader 測試確認通過**

```bash
dotnet test AseAudit.DbTool.Tests/AseAudit.DbTool.Tests.csproj --filter "FullyQualifiedName~ManifestLoaderTests"
```
Expected: 8 個測試全通過（plan 原寫 7 為筆誤）
...
git commit -m "實作 ManifestLoader，含 JSON 反序列化、表名/loadOrder 唯一性與腳本檔存在性驗證（8 個單元測試）"
```
...
- [x] **Step 1：移除 Program.cs 的 Migrate 區段**
...
- [x] **Step 2：刪除整個 Migrations 資料夾**

```bash
git rm -r AseAudit.Infrastructure/Migrations/
```

- [x] **Step 3：移除 EF.Design 套件參考（若有）**
...
實際：18 個全通過（ManifestLoader 8、ModeDetector 4、BackupArchive 5、SmokeTest 1）
